...
function getOpaqueBoundingBox(context, width, height) {
  ...
  const bounds = Array(width * height)
    .fill(0)
    .map((_, index) => {
      return {
        x: index % width,
  ...
  return pixelData
    .map(({ rgbColor }) =>
      [rgbColor.r, rgbColor.g, rgbColor.b]
        .map(byte => String.fromCodePoint(Math.floor(byte)))
        .join("")
    )
    .join("");
}
...
function getInfo(data, metrics) {
  ...
  const info = data.map(point => {
    ...
    const pixelData = drawCharAndGetData(
      context,
      metrics.xOffset,
      metrics.yOffset,
      char,
...
export function getPixelDataForChar(char, metrics, size) {
  ...
  context.fillText(
    char,
    -size * metrics.xOffset,
    size - size * metrics.yOffset
  );
  const pixelData = getPixelData(context, size, size);
  canvas.remove();
  return pixelData;
...
export function getData(data) {
  ...
  const missingGlyphPixelData = getPixelDataForChar(
    String.fromCodePoint(0x124ab),
    metrics,
    8
  );
  ...
  const dataMap = data.reduce((map, point, index) => {
    ...
    const supported =
      infoForPoint.hash !== missingGlyphHash &&
      infoForPoint.width < MAX_CHAR_WIDTH;
...
export function emojiToNameMap(data) {
  return data.reduce(
    (map, point) => {
      const char = point.char;
      const name = getURLSafeName(point.name);
...
export function filterUnsupported(data, appearanceData) {
